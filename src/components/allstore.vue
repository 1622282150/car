<template>
  <div>
    ...
    <div style="position: relative;">
      <van-nav-bar @click-left="onClickLeft" title="全部门店" left-arrow/>
      <span class="address">
        地区
        <i class="iconfont">&#xe606;</i>
      </span>
    ...
    <div class="allstore" v-for="(i,index) in list "  :key=index>
      <div class="allstore-top">
        <img class="allstore-img" :src="i.ShopImg" alt="">
        <div class="allstore-p">
          <p style="font-size:16px;margin:0;padding-top:0.5rem;">{{i.Name}}</p>
          <p style="color:silver;font-size:16px;margin:0;padding-top:0.5rem;">{{i.Mobile}}</p>
    ...
    <van-popup v-model="show" position="bottom" :overlay="true">
      <div class="showdialog">
        ...
        <div class="showdialog-three">
          ...
          <span class="showdialog-spanb">13060575060</span>
        </div>
        <div class="showdialog-four">马上咨询</div>
      </div>
    </van-popup>
  </div>
</template>
...
export default {
  ...
  methods: {
    getlist(){
      request.post(this.$api+"/Wechat/GetUserShoplist",{
